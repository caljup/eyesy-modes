...
def draw(screen, etc):
    global pipe_points, camera_x, camera_y, camera_z
    screen.fill((0, 0, 0))
    camera_x += etc.knob1 * 10
    camera_y += etc.knob2 * 10
    camera_z += etc.knob3 * 10
    for i in range(len(pipe_points) - 1):
        x1, y1, z1 = pipe_points[i]
        x2, y2, z2 = pipe_points[i + 1]
        x1_2d = int(x1 + camera_x) + etc.xres / 2
        y1_2d = int(-y1 + camera_y) + etc.yres / 2
        x2_2d = int(x2 + camera_x) + etc.xres / 2
        y2_2d = int(-y2 + camera_y) + etc.yres / 2
